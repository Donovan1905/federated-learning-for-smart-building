...
def fedDist(nb_loop, nb_clients, testing_floor):
    i = 1
...
def updateGlobalModel(aggregated_weight_matrix, aggregated_bias_matrix, X_train_compare, y_train_compare):
    new_global_model = MLPRegressor(hidden_layer_sizes=(15, ), activation='tanh', solver='adam', alpha=0.0001, random_state=42, max_iter=1000)
    new_global_model.partial_fit(X_train_compare, y_train_compare)
    print("----- UPDATE GLOBAL MODEL -----")
    new_global_model.coefs_ = aggregated_weight_matrix
    new_global_model.intercepts_ = aggregated_bias_matrix
    globals()['global_model'] = new_global_model
...
def federate():
    ...
    for model in local_models:
        weight_matrix.append(calcDistance(model.coefs_, global_model.coefs_))
        bias_matrix.append(calcDistance(model.intercepts_, global_model.intercepts_))
...
def calcDistance(matrix, global_matrix):
    b = global_matrix
    distances = []
    newMatrix = []
    i = 0
    for elem in matrix:
        ...
        p_b = np.array(b[i]).reshape(1, -1)
        distances.append(distance.cdist(elem, p_b, 'euclidean'))
...
def updateLocalModels(loop_number):
    ...
    for model in local_models:
        ...
        i = i+1
    globals()['local_models'] = new_local_models
...
def storeMetrics():
    ...
    results_folder = os.path.join(os.path.dirname(__file__), "../_data/results/aggregation/fedDist/fedDist-" + str(date))
